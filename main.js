(function () {
  ...
  /* ⚠️ 이 파일을 수정하면 각 랜딩 HTML 의 <script src="main.js?v=날짜"> 버전을 반드시 올릴 것.
     ...
     2026-08-31 실제로 이걸로 트래킹이 안 붙어 한참 헤맸다. 현재 v=20260831b. */
  ...
  (function () {
    ...
    if (!sticky) return;
    // 문제공감 마무리 문구(.pain-close)가 화면에 들어오면 등장 → 이후 계속 표시
    var target = document.querySelector(".pain-close") || document.querySelector(".hero");
    if (!target || !("IntersectionObserver" in window)) { sticky.classList.add("is-shown"); return; }
    var so = new IntersectionObserver(function (es) {
      es.forEach(function (en) {
        if (en.isIntersecting) { sticky.classList.add("is-shown"); so.unobserve(en.target); }
      });
